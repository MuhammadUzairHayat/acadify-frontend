...
import {
  useEnrollmentStats,
  useOwnerCoursesForEnrollments,
  useOwnerEnrollments,
} from "@/hooks/useOwnerEnrollments";
...
export function OwnerEnrollmentsPageClient() {
  const queryClient = useQueryClient();
  const { billing } = useBillingData();
  const planLimits = (
    billing as { entitlements?: PlanFeatureFlags; usage?: { limits?: PlanFeatureFlags } } | null
  )?.entitlements ??
    (billing as { usage?: { limits?: PlanFeatureFlags } } | null)?.usage?.limits;
  const hasAdvancedAnalytics = planLimits?.advancedAnalytics ?? false;
  const hasAffiliateTools = planLimits?.affiliateMarketing ?? false;
  ...
  const listFilters = useMemo(
    ...
    [courseId, from, limit, search, sortBy, sortOrder, status, to],
  );

  const { data: courses = [] } = useOwnerCoursesForEnrollments();
  const { data: stats } = useEnrollmentStats(courseId || undefined);
  const {
    ...
    fetchNextPage,
    error: listError,
  } = useOwnerEnrollments(listFilters);

  const enrollments = enrollmentPages?.pages.flatMap((page) => page.data) ?? [];
  const meta = enrollmentPages?.pages[enrollmentPages.pages.length - 1]?.meta ?? null;
  ...
  const displayError =
    error || (listError instanceof Error ? listError.message : listError ? "Failed to load enrollments" : "");
  ...
  const onExportCsv = async () => {
    ...
    const { apiFetch } = await import("@/lib/fetch-client");
    const url = `/enrollments/export/csv${
  ...
  return (
    <OwnerPageShell
      eyebrow="Students"
      title="Enrollments"
      subtitle="Manage across courses."
      action={
        <div className="flex gap-2 flex-wrap justify-end">
          <Button
            ...
            title={hasAdvancedAnalytics ? undefined : "CSV export requires Pro plan or higher"}
          ...
          <Button
            ...
            size="sm"
            className="rounded-xl"
            disabled={!hasAffiliateTools}
            title={hasAffiliateTools ? undefined : "Bulk enrollment requires Starter plan or higher"}
            onClick={openBulk}
          >
            Bulk
          </Button>
          <Button size="sm" className="rounded-xl" onClick={openManual}>
            Enroll
          </Button>
        </div>
      }
    ...
    >
        {!hasAdvancedAnalytics || !hasAffiliateTools ? (
          <Card className="border-accent/30 bg-accent/5">
            <p className="text-sm text-text-secondary">
              {!hasAffiliateTools ? (
                <>
                  <strong className="text-text-primary">Bulk enrollment</strong> is on Starter and
                  above.{" "}
                </>
              ) : null}
              {!hasAdvancedAnalytics ? (
                <>
                  <strong className="text-text-primary">Date filters & CSV export</strong> need Pro
                  or Enterprise.{" "}
                </>
              ) : null}
              <Link href="/owner/billing" className="text-accent hover:underline">
                Upgrade in Billing →
              </Link>
            </p>
          </Card>
        ...
        {/* Infinite scroll */}
        {meta && enrollments.length > 0 ? (
          <div className="flex flex-col items-center gap-3 text-sm text-text-secondary">
            <div>
              Loaded {enrollments.length} of {meta.total} enrollments
            </div>
            <div ref={loadMoreRef} className="flex justify-center py-4 min-h-[48px] w-full">
