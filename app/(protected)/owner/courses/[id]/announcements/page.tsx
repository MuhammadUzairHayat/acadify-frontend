...
export default function CourseAnnouncementsPage() {
  ...
  const fetchAnnouncements = useCallback(async (query?: string) => {
    setLoading(true);
    setError("");
    try {
      const data = await api.course.getAnnouncements(courseId, query);
  ...
  const submit = async () => {
    ...
    try {
      ...
      resetForm();
      await fetchAnnouncements(search);
    } catch (e) {
    ...
    <div>
      <header className="bg-surface border-b border-border sticky top-0 z-10">
        <div className="px-8 py-4">
          <h1 className="text-2xl font-bold text-text-primary">Course Announcements</h1>
          <p className="text-text-secondary text-sm">
            Announcement features are queued for next phase.
          </p>
          <div className="mt-4">
      ...
      <div className="p-8 space-y-6">
        ...
        <Card className="space-y-3">
          ...
          <p className="text-xs text-text-secondary">{filteredCount} announcements found</p>
        </Card>
